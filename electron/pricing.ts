...
async function jdSearchFetch(
  searchUrl: string,
  onBlockDetected?: (win: BrowserWindow) => void,
): Promise<{ offers: import('../src/lib/pricing').PriceOffer[]; finalUrl: string } | null> {
  return new Promise(resolve => {
    let win: BrowserWindow | null = null
    let settled = false

    function finish(result: { offers: import('../src/lib/pricing').PriceOffer[]; finalUrl: string } | null) {
      if (settled) return
      settled = true
      if (win && !win.isDestroyed()) win.destroy()
      win = null
    ...
    win.webContents.once('did-finish-load', async () => {
      ...
      try {
        ...
        `)
        const offers = JSON.parse(offersJson) as import('../src/lib/pricing').PriceOffer[]
...
async function autoCaptureChannel(
  ...
  try {
    ...
    let rawOffers: import('../src/lib/pricing').PriceOffer[]
